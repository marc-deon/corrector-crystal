...
int Player_GetFirstPattern(Player* p){
    ...
    for(int i = 0; i < sb_count(p->pointCharacter->motions); i++){
        ...
        Action* curAct = cb_last(p->pointCharacter->stateHistory).action;
        ...
        if(
            ...
            (motion->priority <= cb_last(p->pointCharacter->stateHistory).action->priority)
            ...
            && !((motion->priority == 1) && cb_last(p->pointCharacter->stateHistory).action->priority == 1)
            ){
            continue;
        }
